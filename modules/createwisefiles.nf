process create_wise_files {
	...
	input:
	tuple val(id), path(on), path(bam),path(bai),path(off)
	...
	paste <(tail -n 1 $on | cut -f 1) <(tail -n 1 $on | cut -f 3 | egrep -o '[1234567890]+') <(cut -d "," -f 2 $off | egrep -o '[1234567890]+') <(samtools view -c -f 4 $bam | egrep -o '[1234567890]+') > ${id}.samplewise.txt
