...
GQfilter(genotype.out[0],GQ_ch,params.DP,params.genome,indexes_ch)
create_run_vcf(GQfilter.out[0].map{id,vcfgz,vcftbi -> vcfgz}.flatten().toList(),GQfilter.out[0].map{id,vcfgz,vcftbi -> vcftbi}.flatten().toList(),run,GQ_ch,params.DP,params.alleles,params.allelesidx)
